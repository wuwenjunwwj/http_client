...
pkgdatadir = $(datadir)/spider-utils
pkgincludedir = $(includedir)/spider-utils
pkglibdir = $(libdir)/spider-utils
pkglibexecdir = $(libexecdir)/spider-utils
...
DIST_COMMON = $(am__configure_deps) $(srcdir)/Makefile.am \
	$(srcdir)/Makefile.in $(srcdir)/package_config.h.in \
	$(top_srcdir)/collect.mk $(top_srcdir)/common.mk \
	$(top_srcdir)/configure _aux/config.guess _aux/config.sub \
	_aux/depcomp _aux/install-sh _aux/ltmain.sh _aux/missing
subdir = .
...
CONFIG_HEADER = package_config.h
CONFIG_CLEAN_FILES =
...
ACLOCAL = ${SHELL} /search/wuwenjun/project/http_client/_aux/missing --run aclocal-1.11
AMTAR = ${SHELL} /search/wuwenjun/project/http_client/_aux/missing --run tar
AR = ar
AUTOCONF = ${SHELL} /search/wuwenjun/project/http_client/_aux/missing --run autoconf
AUTOHEADER = ${SHELL} /search/wuwenjun/project/http_client/_aux/missing --run autoheader
AUTOMAKE = ${SHELL} /search/wuwenjun/project/http_client/_aux/missing --run automake-1.11
AWK = gawk
BUILD_DEPENDENCIES = /search/wuwenjun/project/http_client
CC = gcc -std=gnu99
CCDEPMODE = depmode=gcc3
CFLAGS = -g -O2 -pthread $(if $(DEBUG), -g -O0) $(if $(OPTIMIZE), -O3) $(if \
	$(PROFILE), -pg)
CPP = gcc -std=gnu99 -E
CPPFLAGS =  -D_REENTRANT -D_FILE_OFFSET_BITS=64 -D SS_DOCID_BITS=256 $(if $(OPTIMIZE), -DNDEBUG)
CXX = g++ -std=gnu++0x
...
LN_S = ln -s
LTLIBOBJS = 
MAKEINFO = ${SHELL} /search/wuwenjun/project/http_client/_aux/missing --run makeinfo
MKDIR_P = /bin/mkdir -p
NM = /usr/bin/nm -B
NMEDIT = 
OBJDUMP = objdump
...
PACKAGE_BUGREPORT = songnan@sogou-inc.com
PACKAGE_DEPENDENCIES = 
PACKAGE_NAME = Spider Utils
PACKAGE_STRING = Spider Utils 2.0.0
PACKAGE_TARNAME = spider-utils
...
STRIP = strip
VERSION = 2.0.0
abs_builddir = /search/wuwenjun/project/http_client
abs_srcdir = /search/wuwenjun/project/http_client
abs_top_builddir = /search/wuwenjun/project/http_client
abs_top_srcdir = /search/wuwenjun/project/http_client
...
install_sh = ${SHELL} /search/wuwenjun/project/http_client/_aux/install-sh
...
AM_CXXFLAGS = -Wall -Werror -D SS_THRIFT_VERSION=$(SS_THRIFT_VERSION)
...
SUBDIRS =  src test
all: $(BUILT_SOURCES) package_config.h
	$(MAKE) $(AM_MAKEFLAGS) all-recursive

.SUFFIXES:
am--refresh:
	@:
$(srcdir)/Makefile.in:  $(srcdir)/Makefile.am $(top_srcdir)/common.mk $(top_srcdir)/collect.mk $(am__configure_deps)
	@for dep in $?; do \
...
package_config.h: stamp-h1
	@if test ! -f $@; then \
	  rm -f stamp-h1; \
	  $(MAKE) $(AM_MAKEFLAGS) stamp-h1; \
	else :; fi

stamp-h1: $(srcdir)/package_config.h.in $(top_builddir)/config.status
	@rm -f stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status package_config.h
$(srcdir)/package_config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f stamp-h1
	touch $@

distclean-hdr:
	-rm -f package_config.h stamp-h1
...
TAGS: tags-recursive $(HEADERS) $(SOURCES) package_config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	set x; \
	here=`pwd`; \
	...
	list='$(SOURCES) $(HEADERS) package_config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
	  done | \
	  $(AWK) '{ files[$$0] = 1; nonempty = 1; } \
...
ctags: CTAGS
CTAGS: ctags-recursive $(HEADERS) $(SOURCES) package_config.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	list='$(SOURCES) $(HEADERS) package_config.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
...
all-am: Makefile package_config.h all-local
